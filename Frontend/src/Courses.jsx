...
import {
  BookOpen, CheckCircle, Clock, Search, Plus, Trash2, Edit, X, Image as ImageIcon,
  UploadCloud, AlertCircle, Loader2, RefreshCw, Bell, Trash, XCircle, CheckCircle2, DollarSign, Calendar
} from 'lucide-react';
...
const translations = {
  en: {
    ...
    subtitle: "Create, review, and manage your academy courses",
    ...
    endDate: "End Date",
    endTime: "End Time",
    coverImage: "Drag & Drop Course Image",
    publishBtn: "Publish Course",
    updateBtn: "Update Course",
    search: "Search courses...",
    allStatus: "All Status",
    noCourses: "No Courses Found",
    noCoursesSub: "Create your first course using the form.",
    editingBanner: "You are currently updating an existing course.",
    processing: "Please Wait...",
    uploadingMedia: "Uploading Data & Image...",
    translating: "Translating & Saving...",
    deleteConfirm: "Delete Course? This action cannot be undone.",
  ...
  ta: {
    ...
    subtitle: "உங்கள் பாடநெறிகளை உருவாக்கவும் மற்றும் நிர்வகிக்கவும்",
    totalCourses: "மொத்த பாடநெறிகள்",
    published: "வெளியிடப்பட்டது",
    pending: "நிலுவையில் உள்ளது",
    createCourse: "புதிய பாடநெறியை உருவாக்கு",
    updateCourse: "பாடநெறியை புதுப்பி",
    ...
    coverImage: "பாடநெறி படத்தை பதிவேற்றவும்",
    publishBtn: "பாடநெறியை வெளியிடு",
    updateBtn: "புதுப்பிக்கவும்",
    search: "தேடு...",
    allStatus: "அனைத்தும்",
    noCourses: "பாடநெறிகள் எதுவும் இல்லை",
    noCoursesSub: "படிவத்தைப் பயன்படுத்தி முதல் பாடநெறியை உருவாக்கவும்.",
    editingBanner: "நீங்கள் தற்போது ஒரு பாடநெறியைப் புதுப்பிக்கிறீர்கள்.",
    processing: "காத்திருக்கவும்...",
    uploadingMedia: "தரவு பதிவேற்றப்படுகிறது...",
    translating: "மொழிபெயர்க்கப்படுகிறது...",
    deleteConfirm: "பாடநெறியை நீக்க வேண்டுமா? இந்தச் செயலை செயல்தவிர்க்க முடியாது.",
...
const CourseAdminPanel = () => {
  ...
  const [form, setForm] = useState({
    title: '', description: '', price: '', start_date: '', start_time: '', end_date: '', end_time: ''
  });
  const [imageFile, setImageFile] = useState(null);
  const [existingImage, setExistingImage] = useState(null);
  const [previewUrl, setPreviewUrl] = useState(null); // Added for Image Preview
  ...
  const fetchCourses = useCallback(async () => {
    setLoading(true);
    try {
      const res = await authenticatedFetch(`/all-courses/?lang=${lang}`);
      let fetchedCourses = [];
      if (res && typeof res.json === 'function') {
         const parsed = await res.json();
         fetchedCourses = parsed.data || parsed || [];
  ...
  const handleTextChange = (e) => {
    ...
    // Enhanced Date Formatting flow (DD/MM/YYYY)
    if (name === "start_date" || name === "end_date") {
      let val = value.replace(/\D/g, ""); // Keep only numbers

      if (val.length > 4) {
        value = val.slice(0, 2) + "/" + val.slice(2, 4) + "/" + val.slice(4, 8);
  ...
  const resetForm = () => {
    setForm({ title: '', description: '', price: '', start_date: '', start_time: '', end_date: '', end_time: '' });
    setImageFile(null);
  ...
  const handleEdit = (course) => {
    ...
    setExistingImage(course.image_url || null);
    setPreviewUrl(null); // Reset local preview when editing
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await authenticatedFetch(`/delete/${id}/`, { method: 'DELETE' });
  ...
  const handleStatusChange = async (id) => {
    ...
    try {
      const res = await authenticatedFetch(`/status/replacer/courses/${id}/`, { method: "POST" });
      let parsedRes = res;
      ...
      const successMessage = newStatus === "published"
        ? (lang === "ta" ? "பாடநெறி வெளியிடப்பட்டது" : "Course published successfully")
        : (lang === "ta" ? "பாடநெறி நிலுவைக்கு மாற்றப்பட்டது" : "Course moved to pending status");

      toast.success(successMessage, { id: loadingToast });
      setCourses(prev => prev.map(c => (c.id || c._id) === id ? { ...c, status: newStatus } : c));
    } catch (error) {
      toast.error(lang === "ta" ? "நிலைமாற்றம் தோல்வி" : "Failed to update status", { id: loadingToast });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (imageFile) {
          ...
          throw new Error("Image file is required to create a course.");
      }

      setProcessStep(t.translating);
      const url = isEditing ? `/update/${editId}/` : '/create-courses/';
      ...
      setModalStatus({
        ...
        title: t.success,
        message: isEditing
            ? (lang === 'ta' ? "பாடநெறி வெற்றிகரமாக புதுப்பிக்கப்பட்டது" : "Course Updated Successfully")
            : (lang === 'ta' ? "பாடநெறி வெற்றிகரமாக உருவாக்கப்பட்டது" : "Course Created Successfully")
      });
      setIsModalOpen(true);
      ...
      setModalStatus({
        isSuccess: false,
        title: t.error,
        message: error.message || "Failed to submit course. Please check fields and try again."
      });
      setIsModalOpen(true);
    } finally {
  ...
  const filteredCourses = courses.filter(course => {
    const searchMatch = (course.title || "").toLowerCase().includes(searchFilter.toLowerCase());
    const currentStatus = (course.status || "pending").toLowerCase().trim();
  ...
  const stats = {
    ...
    published: courses.filter(c => (c.status || "").toLowerCase().trim() === "published").length,
    pending: courses.filter(c => (c.status || "pending").toLowerCase().trim() === "pending").length,
  };
      ...
      <div className="admin-layout">
                    ...
                    <div className="notif-list">
                      {notifications.length === 0 ? (
                        <div className="notif-empty">
                          <Bell size={32} className="text-muted" style={{ opacity: 0.5, marginBotto: '0.5rem' }} />
        ...
        <div className="stats-grid" style={{ marginBottom: '2.5rem' }}>
          <div className="stat-card gradient-blue">
            <BookOpen size={24} /><div><h3>{stats.total}</h3><p>{t.totalCourses}</p></div>
          </div>
        ...
        <div className="main-grid">
          <section className="form-section">
            <div className="glass-panel">
              ...
              <h2 className="section-title">{isEditing ? t.updateCourse : t.createCourse}</h2>
              ...
              <form onSubmit={handleSubmit} className="blog-form">
                ...
                <div className="input-group">
                  ...
                  <label className="floating-label">{t.courseTitle} *</label>
                </div>
                ...
                <div className="form-row">
                  <div className="input-group">
                    <input
                      ...
                      required
                    />
                    <label className="floating-label">{t.startDate} *</label>
                  </div>
                  <div className="input-group">
                    <input type="time" name="start_time" className="floating-input" placeholder=" " value={form.start_time} onChange={handleTextChange} required />
                    <label className="floating-label">{t.startTime} *</label>
                  </div>
                </div>
                ...
                <div className="form-row">
                  <div className="input-group">
                    ...
                    required
                  />
                    <label className="floating-label">{t.endDate} *</label>
                  </div>
                  <div className="input-group">
                    <input type="time" name="end_time" className="floating-input" placeholder=" " value={form.end_time} onChange={handleTextChange} required />
                    <label className="floating-label">{t.endTime} *</label>
                  </div>
                </div>
                ...
                <div className="media-uploads">
                  <div className="upload-zone full-width unified-media-zone">
                    ...
                    {/* Render Image Preview or Upload Icon */}
                    ...
                    <p style={{ fontWeight: '600' }}>
                      {imageFile ? imageFile.name : (existingImage ? "Image Uploaded (Click to change)" : t.coverImage)}
                    </p>
                  </div>
                </div>
                ...
                {loading ? (
                  ...
                  filteredCourses.map((course, idx) => (
                    <motion.div
                      ...
                      key={course.id || course._id || idx} className="blog-card"
                    >
                      <div className="blog-card-img">
                        {course.image_url ? <img src={getImageUrl(course.image_url)} alt="Course Image" loading="lazy" /> : <ImageIcon className="text-muted" />}
                      </div>
                      <div className="blog-card-content">
                        <div className="blog-meta">
                          <span className={`status-badge ${(course.status || "pending").toLowerCase()}`}>
                            {(course.status || "pending").toLowerCase() === "published" ? t.published : t.pending}
                          </span>
                        </div>

                        <h3 className="blog-title">{course.title || 'Untitled Course'}</h3>
                        <p className="blog-author" style={{display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden'}}>{course.description}</p>

                        <div className="blog-saved-media" style={{marginTop: '0.5rem'}}>
                          <span className="saved-media-badge media-item" style={{background: '#F0FDF4', color: '#166534', border: '1px solid #BBF7D0'}}>
                            <span>₹{course.price}</span>
                          </span>
                          <span className="saved-media-badge media-item">
                            <Calendar size={12}/> {course.start_date}
                          </span>
                          <span className="saved-media-badge media-item">
                            <Clock size={12}/> {course.start_time}
                          </span>
                        </div>

                        <div className="blog-actions" style={{marginTop: '0.5rem'}}>
                          <button onClick={() => handleStatusChange(course.id || course._id)} className={`btn-icon ${(course.status || 'pending').toLowerCase() === "published" ? "btn-pending" : "btn-publish"}`}>
                            {(course.status || 'pending').toLowerCase() === "published" ? <Clock size={16} /> : <CheckCircle size={16} />}
                          </button>
                          <button onClick={() => handleEdit(course)} className="btn-icon btn-edit" title="Edit Course"><Edit size={16} /></button>
                          <button onClick={() => handleDelete(course.id || course._id)} className="btn-icon btn-delete" title="Delete Course"><Trash2 size={16} /></button>
                        </div>
                      </div>
                    </motion.div>
                  ))
...
const cssStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  .media-uploads { display: grid; grid-template-columns: 1fr; gap: 1rem; }
  ...
  .image-preview { width: 100%; max-height: 200px; object-fit: cover; border-radius: 8px; margin-bottom: 0.5rem; border: 1px solid var(--border); box-shadow: var(--shadow-sm); }
